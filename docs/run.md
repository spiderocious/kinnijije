...
- Ports free locally: **9093** (main-backend), **5173** (web), **5174** (admin-web), **3000** (website), **4173/4174** (vite preview)
...
| -------------- | ---------- | -------- | ---------------------- | ------------------------------ |
| `main-backend` | Express    | 9093     | `pnpm start`           | Public HTTP API (`/api/v1/*`)  |
...
pnpm -F @kinnijije/main-backend dev
pnpm -F @kinnijije/web dev
pnpm -F @kinnijije/admin-web dev
pnpm -F @kinnijije/website dev
...
```bash
pnpm -F @kinnijije/main-backend dev        # 9093
pnpm -F @kinnijije/web dev                 # 5173 → calls main-backend
pnpm -F @kinnijije/website dev             # 3000 → links to web
```
...
```bash
# One app
pnpm -F @kinnijije/main-backend build
pnpm -F @kinnijije/web build
...
```bash
# Express service
pnpm -F @kinnijije/main-backend build && pnpm -F @kinnijije/main-backend start

# Vite preview (production bundle on a preview server, port 4173/4174)
pnpm -F @kinnijije/web build && pnpm -F @kinnijije/web start
pnpm -F @kinnijije/admin-web build && pnpm -F @kinnijije/admin-web start

# Next.js production
pnpm -F @kinnijije/website build && pnpm -F @kinnijije/website start
```
...
```bash
# Single project
pnpm -F @kinnijije/main-backend typecheck
pnpm -F @kinnijije/web lint
...
```bash
curl http://localhost:9093/api/v1/health           # main-backend
```
...
- `Module not found: '@kinnijije/ui'` after a rename — restart the dev server; tsconfig path edits aren't watched.
