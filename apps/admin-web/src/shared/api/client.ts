...
// Thin typed wrapper over the shared ky client. Every backend response is a
// `{ data }` envelope; these helpers unwrap `.data` and rethrow backend errors
// as a typed `ApiError` (code + message), so feature hooks deal in domain types
// and never touch ky or the envelope directly.
...
// Some endpoints (cursor lists) return extra envelope fields alongside `data`;
// this variant returns the whole parsed body so the caller can read them.
...
async function toApiError(err: unknown): Promise<ApiError> {
  // ky throws HTTPError with a `.response`; pull the envelope's error block.
...
export const api = {
  ...
  put: <T>(path: string, json?: unknown): Promise<T> =>
    unwrap<T>(apiClient.put(path, json !== undefined ? { json } : undefined)),
